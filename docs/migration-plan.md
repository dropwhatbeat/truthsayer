# Migration Plan: Absurd Truths — Single-Device → Multiplayer

**Goal**: Add session-based multiplayer (rooms, players, shared game state) to the existing single-device party game. Ship fast, lay groundwork for a future multi-game platform architecture.

**Scope**: 5 concurrent players max. Real-time via polling (upgradeable to Electric SQL later).
...
│  │   (app/api/)             │    │                             │ │
│  │                          │    │   Pages (by phase):         │ │
│  │  POST /rooms             │    │   / → setup/lobby           │ │
│  │  GET  /rooms/:code       │    │   /game/:code/waiting       │ │
│  │  POST /rooms/:code/join  │    │   /game/:code/reading       │ │
│  │  POST /rooms/:code/start │    │   /game/:code/discuss       │ │
│  │  POST /rooms/:code/move  │    │   /game/:code/reveal        │ │
│  │                          │    │   /game/:code/end           │ │
...
- `players.role` — `'guesser' | 'truthsayer' | 'bluffer'` (can be null in lobby, assigned on game start)
...
- `game_moves.move_type` — `'show_secret' | 'reveal_to_all' | 'next_card'` (maps to current phase transitions)
- `game_moves.data` — JSON blob for extensibility (future game-specific payloads)
...
Phase:
  ...
  reading  → rooms.status = 'playing' AND last move = 'show_secret'
  discuss  → rooms.status = 'playing' AND timer expired (computed client-side)
  reveal   → rooms.status = 'playing' AND last move = 'reveal_to_all'
  end      → rooms.status = 'played' OR no more rounds
```

Alternatively, add a `current_phase` column to `rooms` for simplicity. **Decision: add the column.** A party game with 5 players doesn't need derived state elegance — it needs debuggability. When Bob says "I'm stuck on the waiting screen", you want to `SELECT current_phase FROM rooms WHERE code = 'ABC123'` and see the answer.
...
2. Split single page into phase-based routes
...
AbsurdTruthsGame.tsx                   app/
  useState(screen)                       page.tsx (lobby/setup)
  useState(phase)                        game/
  useState(deck)                           [code]/
  useState(index)                            page.tsx (redirect to waiting)
  shuffle()                                  waiting/page.tsx
  handleStart()                              reading/page.tsx
  handleShowSecret()                         discuss/page.tsx
  handleNext()                               reveal/page.tsx
  ...                                        end/page.tsx
  (500+ lines, all in one file)
                                     lib/
...
- API routes:
  ...
  - `app/api/rooms/[code]/join/route.ts` — POST (join room)
  - `app/api/rooms/[code]/start/route.ts` — POST (start game, shuffle cards)
  - `app/api/rooms/[code]/moves/route.ts` — POST (submit move)
...
**Deliverables:**
- Room creation flow: host creates room → gets join code → shares with friends
- Join flow: player enters code + name → joins room
- Lobby: shows player list, host can start game
...
- Multiple game types (only Absurd Truths, but schema is extensible via `deck_type` and `move.data`)
...
│   ├── globals.css
│   ├── providers.tsx
│   ├── page.tsx                      ← Setup/Lobby (was SetupScreen)
│   └── game/
│       └── [code]/
...
├── components/
│   └── absurd-truths/
│       ├── WordCard.tsx              ← Shared card UI
│       ├── CategoryPills.tsx         ← Shared category badges
│       ├── PlayerList.tsx            ← New: lobby player list
│       ├── PhaseControls.tsx         ← New: phase-specific actions
...
- [ ] Players join by entering code + name
