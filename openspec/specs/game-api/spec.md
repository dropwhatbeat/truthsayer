...
Provides Next.js API route handlers for the Truthsayer multiplayer party game room lifecycle — creating rooms, joining/registering players, starting games, and submitting moves — backed by Drizzle ORM and Postgres.
...
The system SHALL accept `POST /api/rooms/[code]/start`, validate at least 3 players are registered, assign roles (1 judge, 1 honest, rest liars), shuffle the deck server-side, insert `game_rounds` rows, and set `current_phase = 'reading'` and `status = 'playing'`.
...
The system SHALL accept `POST /api/rooms/[code]/moves` with `{ playerId, playerSecret, moveType, data }`, validate the move against the current game phase, insert a `game_moves` row, and advance the phase when conditions are met.
